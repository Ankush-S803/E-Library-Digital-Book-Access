...
conn = mysql.connector.connect(host='localhost', user='root', password='Ankush82006#')
cursor = conn.cursor()
...
tables = [
    ...
    "CREATE TABLE Categories (category_id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(100) NOT NULL)",
    """CREATE TABLE Books (
        book_id INT AUTO_INCREMENT PRIMARY KEY, title VARCHAR(200) NOT NULL,
        author_id INT, category_id INT, available_copies INT NOT NULL DEFAULT 0,
        total_copies INT NOT NULL DEFAULT 0, published_year INT, description TEXT,
        isbn VARCHAR(20), publisher VARCHAR(100),
        FOREIGN KEY (author_id) REFERENCES Authors(author_id),
        FOREIGN KEY (category_id) REFERENCES Categories(category_id)
    )""",
    """CREATE TABLE Users (
        user_id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(100) NOT NULL,
        email VARCHAR(100) UNIQUE NOT NULL, password_hash VARCHAR(255) NOT NULL,
        membership_type VARCHAR(20) DEFAULT 'standard', max_books INT DEFAULT 3,
        is_active BOOLEAN DEFAULT TRUE
    )""",
    """CREATE TABLE BorrowRecords (
        record_id INT AUTO_INCREMENT PRIMARY KEY, user_id INT, book_id INT,
        status VARCHAR(20) DEFAULT 'active', borrowed_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        due_date DATETIME, returned_at DATETIME,
        FOREIGN KEY (user_id) REFERENCES Users(user_id),
        FOREIGN KEY (book_id) REFERENCES Books(book_id)
    )""",
    """CREATE TABLE Wishlist (
        wishlist_id INT AUTO_INCREMENT PRIMARY KEY, user_id INT, book_id INT,
    ...
    """CREATE TABLE Reviews (
        review_id INT AUTO_INCREMENT PRIMARY KEY, user_id INT, book_id INT,
        rating INT, comment TEXT, reviewed_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES Users(user_id),
        FOREIGN KEY (book_id) REFERENCES Books(book_id),
        UNIQUE (user_id, book_id)
    )""",
    """CREATE TRIGGER after_borrow_insert AFTER INSERT ON BorrowRecords FOR EACH ROW BEGIN
        IF NEW.status = 'active' THEN
            UPDATE Books SET available_copies = available_copies - 1 WHERE book_id = NEW.book_id;
        END IF;
    END;""",
    """CREATE TRIGGER after_borrow_update AFTER UPDATE ON BorrowRecords FOR EACH ROW BEGIN
    ...
    "INSERT INTO Authors (name) VALUES ('J.K. Rowling'), ('George R.R. Martin'), ('J.R.R. Tolkien'), ('F. Scott Fitzgerald'), ('Jane Austen')",
    "INSERT INTO Categories (name) VALUES ('Fantasy'), ('Fiction'), ('Romance'), ('Sci-Fi'), ('Mystery')",
